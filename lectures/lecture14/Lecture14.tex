...
\begin{frame}{Noise conditioned score network}
	...
	\begin{minipage}{0.5\linewidth}
		...
		\begin{block}{Training: loss function}
			\vspace{-0.5cm}
			\[
			\sum_{i=1}^L \sigma_l^2\bbE_{\pi(\bx')} \bbE_{\bepsilon}\bigl\| \bs_l - \bepsilon \bigr\|^2_2,
			\]
			where $\bs_l = \bs(\bx' + \sigma_l \cdot \bepsilon, \btheta, \sigma_l)$.
		\end{block}
	\end{minipage}%
	\begin{minipage}{0.5\linewidth}
...
\begin{frame}{Gaussian diffusion model as VAE}
	\vspace{-0.3cm}
	\[
		\cL_t = \bbE_{\bepsilon} \left[ \frac{1}{2\tilde{\beta}_t} \left\| {\color{violet} \frac{1}{\sqrt{1 - \beta_t}}\left( \bx_t - \frac{\beta_t}{\sqrt{1 - \bar{\alpha}_t} } \bepsilon \right)} - \bmu(\bx_t, \btheta, t) \right\|^2 \right]
	\]
	\vspace{-0.3cm}
	...
	\begin{block}{KL term}
		\vspace{-0.7cm}
		\begin{multline*}
			\cL_t = \bbE_{\bepsilon} \left[ \frac{1}{2\tilde{\beta}_t} \left\|\frac{1}{\sqrt{1 - \beta_t}}\left( \bx_t - \frac{\beta_t}{\sqrt{1 - \bar{\alpha}_t} } \bepsilon \right) - \bmu(\bx_t, \btheta, t) \right\|^2 \right] \\ 
			= {\color{olive}\bbE_{\bepsilon}} \left[ \frac{\beta_t^2}{2\tilde{\beta}_t (1 - \beta_t) (1 - \bar{\alpha_t})} \left\| {\color{violet}\bepsilon} - {\color{teal}\bepsilon(\bx_t, \btheta, t)}\right\|^2 \right]
		\end{multline*}
		\vspace{-0.5cm}
	\end{block}
