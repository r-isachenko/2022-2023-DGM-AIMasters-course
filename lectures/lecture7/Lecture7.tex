...
\begin{frame}{Discrete data vs continuous model}
	\begin{block}{Continuous model}
		\begin{itemize}
			...
			\end{itemize}
			\item Continuous distribution know numerical relationships.
		\end{itemize}
	\end{block}
...
\begin{frame}{Uniform discretization}
	...
	\begin{block}{Proof}
		...
		{\small
		\begin{multline*}
			\bbE_{\pi} \log p(\bx | \btheta) = \int \pi(\bx) \log p(\bx | \btheta) d \by = \sum \Pi(\by) \int_{U[0,1]} \log p(\by + \bu | \btheta) d \bu \leq \\
...
\begin{frame}{Variational dequantization}
	...
	\begin{block}{Flow++: flow-based variational dequantization}
		Let $\bu = g(\bepsilon, \bx, \blambda)$ is a flow model with base distribution $\bepsilon \sim p(\bepsilon)$:
		\vspace{-0.3cm}
		\[
		q(\bu | \bx) = p(f(\bu, \bx, \blambda)) \cdot \left| \det \frac{\partial f(\bu, \bx, \blambda)}{\partial \bu}\right|.
		\]
		\vspace{-0.3cm}
		\[
...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item We could use flows to make variational posterior more expressive. This is equivalent to the flow-based prior in some sort. 
	\end{itemize}
\end{frame}
