...
\begin{frame}{Normalizing flow as VAE}
	\begin{block}{Proof}
		\begin{enumerate}
			...
			\[
				\bbE_{\delta(\bx - \by)} f(\bx) d \bx = \int \delta(\bx - \by) f(\bx) = f(\by)
			\]
			...
			\[
				\log p(\bx | \btheta) = \cL(\bphi, \btheta) + {\color{olive}KL(q(\bz | \bx, \bphi) || p(\bz | \bx, \btheta))}
			\]
		\end{enumerate}
	\end{block}
	\myfootnotewithlink{https://arxiv.org/abs/2007.02731}{Nielsen D., et al. SurVAE Flows: Surjections to Bridge the Gap between VAEs and Flows, 2020}
...
\begin{frame}{Normalizing flow as VAE}
	\begin{block}{Proof}
		...
		\begin{enumerate}
			...
			\vspace{-0.3cm}
			\[
				{\color{teal}\bbE_{q(\bz | \bx)} \log p(\bz)} = \int \delta (\bz - f(\bx)) p(\bz) d \bz = \log p(f(\bx)).
			\]
			\vspace{-0.6cm}
			\item CoV theorem and Bayes theorem:
			\vspace{-0.2cm}
			\[ 
				{\color{violet}\bbE_{q(\bz | \bx, \bphi)} \log \frac{p(\bx | \bz, \btheta)}{q(\bz | \bx, \bphi)}} = \bbE_{q(\bz | \bx, \bphi)} \log \frac{p(\bz | \bx) |\det (\bJ_f)|}{q(\bz | \bx, \bphi)} = \log |\det \bJ_f|.
			\]
			\vspace{-0.6cm}
			\item Log-likelihood decomposition
			\vspace{-0.3cm}
			\[
				\log p(\bx | \btheta) = \cL(\btheta) = \log p(f(\bx)) +  \log |\det \bJ_f|.
			\]
		\end{enumerate}
	\end{block}
	\myfootnotewithlink{https://arxiv.org/abs/2007.02731}{Nielsen D., et al. SurVAE Flows: Surjections to Bridge the Gap between VAEs and Flows, 2020}
...
\begin{frame}{Summary}
	\begin{itemize}
		\item Gaussian autoregressive flow is an autoregressive model with triangular Jacobian. It has fast inference function and slow generation function.
		\vfill
		\item Inverse gaussian autoregressive flow generate new samples fast, but the inference is slow.
		\vfill
